...
DISTRIBUTION_ID="EDIIT73PU1Z4B"  # user-dev
S3_BUCKET="s3-dev-user-front-static"
...
unset EVAL
tr -d '\r' < ${BASE_DIR}/modifyFiles | grep 's3://' | grep -Ev '\.do|\.jsp' | awk -F 's3:\\/\\/s3-dev-user-front-static\\/static' '{print $2}' | while IFS= read -r line; do PATHS_STR="${PATHS_STR} $line"; echo $PATHS_STR > ${BASE_DIR}/paths.str ; done
